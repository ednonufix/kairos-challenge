...
public class SecurityConfig {
	...
	RegisteredClientRepository registeredClientRepository(TokenSettings tokenSettings) {
		var oidcClient = RegisteredClient.withId(UUID.randomUUID().toString())
			...
			.redirectUri("http://localhost:8080/login/oauth2/code/prices-api")
	...
	JWKSource<SecurityContext> jwkSource() {
		RSAKey rsaKey = new RSAKey.Builder(publicKey).privateKey(privateKey).keyID("kairos-challenge").build();
